...
colorPicker = {
	...
	constructor = function( info )
		...
		colorPicker.children.A = guiCreateEdit(info.selectWindow.rgbX + 10, info.selectWindow.rgbY + info.selectWindow.rgbHeight*3+50, info.selectWindow.rgbWidth, info.selectWindow.rgbHeight, tostring( colorPicker.value[4] ), false, colorPicker.GUI.selectWindow)
		...
		colorPicker.GUI.labelA = guiCreateLabel(info.selectWindow.rgbX - 25, info.selectWindow.rgbY + 30 + info.selectWindow.rgbHeight*3,
		...
		lastX,lastY=guiGetPosition(colorPicker.GUI.selectWindow,false)
		...
		colorPicker.GUI.historyLabel = guiCreateLabel(info.selectWindow.historyX, info.selectWindow.historyY+10,
	...
	openSelect = function( currentColor )
		...
		local settx,setty=guiGetPosition(window,false)
		if doesCollide(wx,wy,330,390,settx,setty,395,300) then
			guiSetPosition(colorPicker.GUI.selectWindow,(wx-sx/2)%(sx-330),wy,false)
		end
		colorPicker.currentColor = currentColor
		local r, g, b, a =unpack(colorPicker.convertColorToTable(_G[currentColor]))
		colorPicker.setValue( { r, g, b, a } )
...
addEventHandler('onClientGUIMove',colorPicker.GUI.selectWindow,function()
	local x,y=guiGetPosition(source,false)
	local settx,setty=guiGetPosition(window,false)
	if doesCollide(x,y,330,390,settx,setty,395,300) then
		if doesCollide(0,lastY,sx,390,settx,setty,395,300) then
			x=(lastX>settx) and settx+395 or settx-330
		else--if doesCollide(lastX,0,,390,settx,setty,395,300)
			y=(lastY>setty) and setty+300 or setty-390
		end
		guiSetPosition(source,x,y,false)
	else
		lastX,lastY=x,y
	end
end,false)
...
function onStart()
	...
	bindKey('F3','down',showSettings,true)
...
function renderFrame()
	if doRender and not isPlayerMapVisible() then
		...
		if doDrawRadar then
			...
			else --yes i know it is sooo inefficent, but literally theres no other way then changing the mask otherwise, and i certainly wont do that, and as its used in interiors only its not that bad(render target is deleted in normal world).might think of something else later.
			...
			if not (airmode or parachute) then
				dxDrawImage(size*1.5-blipDimension,size*1.5-blipDimension,blipDimension*2,blipDimension*2,'images/blips/2.png',rot-prz,0,0,normCol,false) --player blip
			end
			local Nx,Ny=getPointFromDistanceRotation(size*1.5,size*1.5,size*0.9,-rot-180)
			dxDrawImage(Nx-blipDimension,Ny-blipDimension,blipDimension*2,blipDimension*2,'images/blips/4.png',0,0,0,normCol,false) --north blip
			blipimages={}
			dxSetRenderTarget()
	...
	if doFPS then
		...
		dxDrawText("FPS:"..tostring(math.ceil(fps)),2,sy-14,sx,sy,normCol,0.93)
	end
end
...
function prepareBlips(px,py,pz,rot,radiusR)
	...
	for _,blip in ipairs(blips) do
		if getElementInterior(localP)==getElementInterior(blip) and getElementDimension(localP)==getElementDimension(blip) then
			...
			local bSize=math.min(getBlipSize(blip),4)*blipDimension
			...
			and (localP~=target and a~=0 and ( not visDist or visDist>getDistanceBetweenPoints3D(px,py,pz,bx,by,bz) )) then --alpha above 0 and distance is close enough
